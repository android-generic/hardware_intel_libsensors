...
int AccelSensor::processEvent(unsigned char *raw_data, size_t raw_data_len){
    ...
    mPendingEvent.data[0] = mPendingEvent.acceleration.x =
        (CONVERT_A_G_VTF16E14_X(GetChannelBytesUsedSize(CHANNEL_X), GetScaleValue(), sample->accel_x))/1000.0;
    mPendingEvent.data[1] = mPendingEvent.acceleration.y =
        (CONVERT_A_G_VTF16E14_Y(GetChannelBytesUsedSize(CHANNEL_Y), GetScaleValue(), sample->accel_y))/1000.0; 
    mPendingEvent.data[2] = mPendingEvent.acceleration.z =
        (CONVERT_A_G_VTF16E14_Z(GetChannelBytesUsedSize(CHANNEL_Z), GetScaleValue(), sample->accel_z))/1000.0;
